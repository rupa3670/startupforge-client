...
const CollaboratorApplications = () => {
    ...
    useEffect(()=>{
        if(!isPending && session?.user?.email){
            fetch(`${process.env.NEXT_PUBLIC_API_URL}/my-application?email=${session.user.email}`)
            .then((res)=>res.json())
            .then((data)=>{
...
if (applications.length=== 0){
     return <p className="p-6 text-center text-gray-500 dark:text-gray-400">No applications yet.</p>;
}
                ...
                {applications.map((app) => (
                    ...
                    >
                        <div className="flex items-start justify-between">
                            ...
                            <span className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap ${statusStyle(app.status)}`}>
                                {app.status}
                            </span>
                        </div>
